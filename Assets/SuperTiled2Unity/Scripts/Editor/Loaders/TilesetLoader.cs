    ...
    public class TilesetLoader
    {
        ...
        private void ProcessTileElement(XElement xTile)
        {
            ...
            if (m_TilesetScript.TryGetTile(index, out tile))
            {
				...
				// Add a collision object that fully covers the tile if the "solid" property is present and set true 
				if (tile.GetPropertyValueAsBool("solid")) {
					ProcessSolidCollision(tile);
				}
            }
        }
		...
		private void ProcessSolidCollision(SuperTile tile) {
			if (tile.m_CollisionObjects == null) {
				tile.m_CollisionObjects = new List<CollisionObject>();
			}
				
			// No object or template, so just set defaults.
			var collision = new CollisionObject();
			collision.m_ObjectId = 0;
			collision.m_ObjectName = "Object_SolidCollision";
			collision.m_ObjectType = "Solid";
			collision.m_Position.x = 0f;
			collision.m_Position.y = 0f;
			collision.m_Size.x = tile.m_Width;
			collision.m_Size.y = tile.m_Height;
			collision.m_Rotation = 0f;
